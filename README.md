...
- **Real-time-based speed control.** The speed factor is simulated seconds per real second, so the scale is always explicit: pick from `1s = 20 min`, `1s = 1 h`, `1s = 6 h`, `1s = 12 h`, `1s = 24 h` (the default, 86,400×), `1s = 1 month`, `1s = 3 months`, `1s = 1 year`, or `1s = 3 years`. Months are 30 days and years 365 days. Pause/resume sits beside it.
...
- **3D mode.** Renders the system with WebGL (Three.js): planets and comets on their real inclined orbits (J2000 inclination + ascending node — Pluto's 17° tilt is unmistakable), texture-mapped Sun, planets, and Moon with a Saturn ring, and an asteroid belt with realistic inclination scatter. Navigate with orbit-around-the-Sun controls: drag to rotate, wheel to zoom toward the cursor, right-drag to pan, double-click to re-center (touch: 1-finger rotate, 2-finger pinch/pan). Planets and the Sun also rotate about their own axes at their real sidereal rates and real axial tilts — Uranus lies on its side at 97.8°, and Venus, Uranus and Pluto turn retrograde — whenever the speed scale is below `1s = 1 month`; above that the spin would strobe, so it freezes while the orbits keep running. The starting meridian is not calibrated, so the time of day a texture shows is decorative. The Three.js renderer loads lazily on first use, so the 2D experience pays no bundle cost.
...
- The 3D overview and comet frames account for viewport aspect and field of view, including portrait screens. Selecting Sun frames it up close, just like a planet; the empty picker option or double-click releases the focus back to the overview.
...
- `Camera` maps world coordinates to screen coordinates; resizing preserves the viewed center and zoom; switching modes re-fits the camera and rebuilds the belt while preserving the clock and date.
